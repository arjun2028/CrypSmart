...
struct CoinRowView: View {
    ...
    var body: some View {
        HStack(spacing: 0){
            Text("\(coin.rank)").font(.caption)
                .foregroundStyle(Color.theme.secondaryText)
                .frame(minWidth: 30)
            Circle().frame(width: 30,height: 30)
            Text(coin.symbol.uppercased()).font(.headline)
                .padding(.leading,6)
                .foregroundStyle(Color.theme.ascent)
            Spacer()
            if showHolding{
                VStack{
                    Text(coin.currentHoldingsValue.asCurrenctWith2Decimal()).bold()
                    Text((coin.currentHoldings ?? 0).asNumberString())
                }
                .foregroundStyle(Color.theme.ascent)
            }
            VStack(alignment: .trailing){
                Text(coin.currentPrice.asCurrenctWith6Decimal())
                    .bold()
                    .foregroundStyle(Color.theme.ascent)
...
#Preview {
    CoinRowView(coin: CoinModel(
        ...
        currentHoldings: 1.5),showHolding: true)
}
